...
3b. **Declare each node's knowledge KIND** — `"kind": "concept" | "procedure" | "fact"` (docs/11; KLI's rough mapping — facts pair with retrieval+spacing, concepts with sense-making, procedures with worked examples and practice). The CONTENT decides, never the domain: `procedure` means *executed on fresh instances* — an integral, a `git rebase`, a statistical-test choice, a conjugation — in any topic whatsoever; most nodes in most topics remain `concept`. `fact` is the kind-form of `arbitrary: true` (emit both). **For every `procedure` node, also emit:**
   ...
   - `practice`: `{"problem_frame": "<what varies and what stays across fresh instances, one paragraph, with bounds that hold difficulty roughly fixed>", "discriminates_from": [<confusable sibling procedure ids — the ones a learner must CHOOSE between; drives interleaved practice>], "verify": "<how to check an answer by direct computation/substitution>", "error_bank": [≤3 of {"error": "...", "misconception": "..."}]}`.
